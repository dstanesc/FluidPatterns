...
import { retrieveArrayProperty, retrieveValueProperty, createDiceProperty, initPropertyTree, activateSlice, activateView } from "./diceArrayApi";

import { DiceArrayController } from "./diceArrayController";
import { DiceArrayBinderHandle } from './diceArrayAdapters';
...
export default function App() {
  ...
  // Controller
  const [diceArrayController, setDiceArrayController] = useState<DiceArrayController>();

  //Binder handles
  const [diceArrayBinderHandle, setDiceArrayBinderHandle] = useState<DiceArrayBinderHandle>();
  ...
  useEffect(() => {
    ...
    async function initWorkspace() {
      ...
      setDiceArrayController(myDiceArrayController);
  ...
  const toggleSlicing = (start: number, end: number) => {
    ...
    if(diceArrayBinderHandle){
      diceArrayBinderHandle.destroy();
    }
    let viewBinderHandle;
    if (sliceToggle) {
      setSliceToggle(undefined);
      viewBinderHandle = activateView(dataBinder, diceArrayController);
    } else {
      setSliceToggle({ start, end });
      viewBinderHandle = activateSlice(dataBinder, diceArrayController, start, end);
    }
    setDiceArrayBinderHandle(viewBinderHandle);
  }
